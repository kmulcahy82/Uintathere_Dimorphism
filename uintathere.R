setwd("~/Desktop/plosRound4Files/CodeAndData/")
...
#Mann-Whitney U Tests for differences between purported males and females 
#for each metric

UintatheresSexed <- read.csv("Uintathere_Measurements_With_Canine_Width.csv", check.names = F) #load dataframe
UintatheresSexed <- UintatheresSexed[UintatheresSexed$Specimen != "YPM VP 011039L",] #remove damaged view of YPM 11039
rownames(UintatheresSexed) <- UintatheresSexed[,1] #rename rows as specimen numbers
uinFem <- UintatheresSexed$Specimen[UintatheresSexed$sex=="F"]
uinMale <- UintatheresSexed$Specimen[UintatheresSexed$sex=="M"]
uinNB <- UintatheresSexed$Specimen[UintatheresSexed$sex=="?"]
UintatheresSexed <- UintatheresSexed[,4:14] #remove helper variables
UintatheresSexed <- UintatheresSexed[,-7] #remove old variable
UintatheresSexed <- UintatheresSexed[UintatheresSexed$sex != "?",] #only include specimens for which sex has been suggested
UintatheresSexed$sex <- as.factor(UintatheresSexed$sex)
#Uintatheres[1,]$alvCan <- NA #This canine is potentially pathologic so it's inclusion is questionable

wilcox.test(nasOcc ~ sex,
            data = UintatheresSexed, 
            alternative = "less")
wilcox.test(nasOcc ~ sex,
            data = UintatheresSexed, 
            alternative = "less")
wilcox.test(nasCon ~ sex,
            data = UintatheresSexed, 
            alternative = "less")
wilcox.test(preCon ~ sex,
            data = UintatheresSexed, 
            alternative = "less")
wilcox.test(occCon ~ sex,
            data = UintatheresSexed, 
            alternative = "less")
wilcox.test(parPro ~ sex,
            data = UintatheresSexed, 
            alternative = "less")
wilcox.test(maxAlv ~ sex,
            data = UintatheresSexed, 
            alternative = "less")
wilcox.test(alvCan ~ sex,
            data = UintatheresSexed, 
            alternative = "less")
wilcox.test(diaLen ~ sex,
            data = UintatheresSexed, 
            alternative = "less")
wilcox.test(canWid ~ sex,
            data = UintatheresSexed, 
            alternative = "less")
...
###########################USING PCA AND PERMANOVA TO SEE IF PURPORTED MALES AND FEMALES DIFFER

### FIRST NEED TO IMPUTE MISSING DATA WITH ONLY SEXED SKULLS

UintatheresSexedScaled <- scale(UintatheresSexed[,1:9])
UintatheresSexedScaled <- as.data.frame(UintatheresSexedScaled)

ut.ncp <- estim_ncpPCA(UintatheresSexedScaled, method.cv= "Kfold")
ncp <- as.numeric(ut.ncp$ncp[1]) 

uintatheresSexedImputed <- imputePCA(UintatheresSexedScaled, ncp = ncp)
uintatheresSexedImputed <- uintatheresSexedImputed$completeObs
uintatheresSexedImputed <- as.data.frame(uintatheresSexedImputed)
uintatheresSexedImputed$sex <- UintatheresSexed$sex

### PCA

sexedPCA <- PCA(uintatheresSexedImputed[,1:9], scale.unit=F)

###########################RUNNING PERMANOVA TO SEE IF PC SCORE IS AFFECTED BY SEX

sexedPCA$eig
sigCols <- sexedPCA$ind$coord[,1:3]
sigCols <- as.data.frame(sigCols)
sigCols$sex <- NA
sigCols$sex <- ifelse(rownames(sigCols) %in% uinFem, "F", 
                      ifelse(rownames(sigCols) %in% uinMale, "M", "?"))

sigCols$sex <- as.factor(sigCols$sex)
adonis2(sigCols[,c("Dim.1", "Dim.2", "Dim.3")] ~ sex, data = sigCols, method = "euc")
