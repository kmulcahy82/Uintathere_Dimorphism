...
#########Load and manipulate bison data this time only retaining skulls with sex metadata

bisonSexed <- bisonSex[bisonSex$sex != "?",]
bisonSexed$sex <- as.factor(bisonSexed$sex)
set.seed(123)
bisonMale <- bisonSexed[bisonSexed$sex == "M",]
bisonFem <- bisonSexed[bisonSexed$sex == "F",]
bisFem <- rownames(bisonFem)
bisonMale[sample((1:7), 4, replace = F), ]
bisMale <- rownames(bisonMale)
bisonFem[sample((1:9), 4, replace = F), ]
bisonSub <- rbind(bisonMale[sample((1:7), 4, replace = F), ], bisonFem[sample((1:9), 4, replace = F), ])

wilcox.test(basFro ~ sex,
            data = bisonSub,
            alternative = "less")
wilcox.test(ectEct ~ sex,
            data = bisonSub,
            alternative = "less")
wilcox.test(proNas ~ sex,
            data = bisonSub,
            alternative = "less")
wilcox.test(nasNuc ~ sex,
            data = bisonSub,
            alternative = "less")
wilcox.test(basTip ~ sex,
            data = bisonSub,
            alternative = "less")
wilcox.test(ectPro ~ sex,
            data = bisonSub,
            alternative = "less")
wilcox.test(basWid ~ sex,
            data = bisonSub,
            alternative = "less")
...
bisScale <- scale(bisonAdults)

bis.ncp <- estim_ncpPCA(bisScale, method.cv= "Kfold")
...
bis.impute <- imputePCA(bisScale, ncp = ncp)
...
normPCA <- PCA(bisScale[,1:7], scale.unit = F)
...
###########################USING PCA AND PERMANOVA TO DETERMINE IF PC SCORES VARY
### WITH SEX

### FIRST NEED TO IMPUTE MISSING DATA

bisonSexScaled <- scale(bisonSub[,1:7])
bisonSexedImputed <- imputePCA(bisonSexScaled)

bisonSexedImputed <- imputePCA(bisonSexScaled, ncp = ncp)
bisonSexedImputed <- bisonSexedImputed$completeObs
bisonSexedImputed <- as.data.frame(bisonSexedImputed)
bisonSexedImputed$sex <- bisonSub$sex

### PCA

sexedPCA <- PCA(bisonSexedImputed[,1:7], scale.unit=F)
